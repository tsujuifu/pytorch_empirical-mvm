...
class MotionEncoder(nn.Module):
    ...
    def forward(self, flow, corr_features):
        ...
        flow_orig = flow
        # safeguard fp16
        flow = flow.to(dtype=corr_features.dtype)
...
class CorrBlock(nn.Module):
    ...
    def index_pyramid(self, centroids_coords):
        ...
        delta = torch.stack(torch.meshgrid(di, dj), dim=-1).to(centroids_coords.device)
...
class RAFT(nn.Module):
    ...
    def forward(self, image1, image2, num_flow_updates: int = 12):
        ...
        # safeguard fp16
        coords0 = coords0.to(dtype=image1.dtype)
        coords1 = coords1.to(dtype=image1.dtype)
